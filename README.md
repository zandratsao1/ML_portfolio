...
Project OverviewThis project aims to help Waze proactively identify users likely to stop using the app (churn). By predicting at-risk users, Waze can implement targeted retention strategies—such as fuel discounts or personalized notifications—to improve long-term user loyalty.

The Problem
Waze leadership wants to understand why users churn and build a model that can predict this behavior before it happens. Initial analysis showed that user activity (measured by activity_days) is a primary indicator of churn, but simple linear relationships are insufficient to capture complex behavioral shifts.

Solution & MethodologyI followed the PACE (Plan, Analyze, Construct, Execute) framework to develop and evaluate three distinct models:
...
Future ImprovementsTo move beyond the current performance, I recommend the following enhancements:
